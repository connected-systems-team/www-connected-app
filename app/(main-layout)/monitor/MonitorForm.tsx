...
import { PortStateValue, PortScanFlowNodeResultStatus } from '@project/source/api/GraphQlGeneratedCode';
...
export function MonitorForm(properties: MonitorFormInterface) {
    ...
    function handleSubmit() {
        ...
        const emailDeliveryPreference: PortScanFlowNodeResultStatus[] = [];
        if(notifyOnSuccess) emailDeliveryPreference.push(PortScanFlowNodeResultStatus.Success);
        if(notifyOnFailure) emailDeliveryPreference.push(PortScanFlowNodeResultStatus.Failure);
        if(notifyOnMismatch) emailDeliveryPreference.push(PortScanFlowNodeResultStatus.Mismatch);
